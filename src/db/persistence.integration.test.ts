...
beforeAll(async () => {
  ...
  await db()
    ...
    .values({
      id: 'test-install',
      githubInstallationId: 'test-install',
      accountLogin: 'test',
      accountType: 'User',
    })
    .onConflictDoNothing();
...
test('replayed and concurrent facts produce one PR and one projection', async () => {
  const input = {
    ...
    sourceUpdatedAt: new Date(instant),
    facts: {
      openedAt: instant,
      mergedAt: null,
      closedAt: instant,
  ...
  expect(
    await db().select().from(s.pullRequests).where(eq(s.pullRequests.id, input.id)),
  ).toHaveLength(1);
  const projections = await db()
    .select()
    .from(s.prMetrics)
    .where(eq(s.prMetrics.pullRequestId, input.id));
  expect(projections).toHaveLength(1);
  expect(projections[0].firstPassGreen).toBeNull();
  const version = await setGatePolicy('test-repo', [{ appId: '1', name: 'unit' }]);
  const [updated] = await db()
    .select()
    .from(s.prMetrics)
    .where(eq(s.prMetrics.pullRequestId, input.id));
  expect(updated.gatePolicyVersion).toBe(version);
});
test('duplicate delivery IDs persist one raw event', async () => {
  ...
  const [a, b] = await Promise.all([
    persistEvent(delivery, 'ping', { hello: 'world' }, {}),
    persistEvent(delivery, 'ping', { hello: 'world' }, {}),
  ]);
  expect(a.id).toBe(b.id);
  expect(
    await db().select().from(s.githubEvents).where(eq(s.githubEvents.deliveryId, delivery)),
  ).toHaveLength(1);
});
test('failed dispatch is recoverable and processing is idempotent', async () => {
  const { persistEvent } = await import('./queries/events');
  const { dispatchEvent } = await import('../inngest/dispatch');
  const { processEvent } = await import('../inngest/process-event');
  const event = await persistEvent(`retry-${Date.now()}`, 'ping', {}, {});
  await expect(
    dispatchEvent(event.id, async () => {
      throw new Error('network unavailable');
    }),
  ).rejects.toThrow();
  let sent = 0;
  await dispatchEvent(event.id, async () => {
    sent++;
  });
  await dispatchEvent(event.id, async () => {
    sent++;
