...
export const stableId = (...values: unknown[]) => createHash('sha256').update(JSON.stringify(values)).digest('hex');
...
async function project(tx: Transaction, id: string, repositoryId: string, facts: PullRequestFacts){
 const [policy]=await tx.select().from(s.gatePolicies).where(eq(s.gatePolicies.repositoryId,repositoryId)).orderBy(desc(s.gatePolicies.version)).limit(1);
 ...
 await tx.insert(s.prMetrics).values(values).onConflictDoUpdate({target:s.prMetrics.pullRequestId,set:values});
}
export async function persistPr(input: PrInput){
 return db().transaction(async tx=>{
  ...
  const [existing]=await tx.select().from(s.pullRequests).where(eq(s.pullRequests.id,input.id));
  const fresh=!existing||input.sourceUpdatedAt>=existing.sourceUpdatedAt;
  const base=fresh?input.facts:existing.facts;
  const combined:PullRequestFacts={...base,checks:canonicalChecks([...(existing?.facts.checks??[]),...input.facts.checks])};
  // Keep observed revision edges even when a force push removes them from API history.
  combined.revisions=[...new Map([...(existing?.facts.revisions??[]),...input.facts.revisions].map(r=>[stableId(r.sha,r.previousSha,r.observedAt),r])).values()];
  const row={...(fresh?input:existing),facts:combined,updatedAt:new Date()};
  await tx.insert(s.pullRequests).values(row).onConflictDoUpdate({target:s.pullRequests.id,set:row});
  for(const revision of combined.revisions){
   await tx.insert(s.commits).values({id:stableId(input.id,revision.sha),pullRequestId:input.id,sha:revision.sha}).onConflictDoNothing();
  ...
  for(const check of combined.checks){
   ...
   await tx.insert(s.observations).values({id:stableId(checkId,check),ciCheckId:checkId,payload:check}).onConflictDoNothing();
  }
  const groups=[{files:combined.files,sha:null,from:null,provenance:'pr-cumulative'},...combined.revisions.map(r=>({files:r.files,sha:r.sha,from:r.previousSha,provenance:'revision-comparison'}))];
  for(const group of groups) for(const file of group.files){
   const values={id:stableId(input.id,group.provenance,group.sha,group.from,file.path),pullRequestId:input.id,commitSha:group.sha,fromSha:group.from,provenance:group.provenance,path:file.path,changeType:file.changeType,additions:file.additions,deletions:file.deletions,normalized:file};
   await tx.insert(s.changedFiles).values(values).onConflictDoUpdate({target:s.changedFiles.id,set:values});
  }
...
export async function recomputePr(id:string){
 await db().transaction(async tx=>{
  const [pr]=await tx.select().from(s.pullRequests).where(eq(s.pullRequests.id,id));if(!pr) throw new Error('PR not found');
...
export async function setGatePolicy(repositoryId:string,gates:Gate[]){
 return db().transaction(async tx=>{
  ...
  const [previous]=await tx.select().from(s.gatePolicies).where(eq(s.gatePolicies.repositoryId,repositoryId)).orderBy(desc(s.gatePolicies.version)).limit(1);
  const version=(previous?.version??0)+1;
  await tx.insert(s.gatePolicies).values({repositoryId,version,gates});
  const prs=await tx.select().from(s.pullRequests).where(eq(s.pullRequests.repositoryId,repositoryId));
