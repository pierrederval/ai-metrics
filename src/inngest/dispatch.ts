...
export type SendEvent=(event:{id:string;name:string;data:{eventId:string}})=>Promise<unknown>;
export async function dispatchEvent(id:string,send:SendEvent=event=>inngest.send(event)){
 ...
 await send({id:event.deliveryId,name:'github/webhook.received',data:{eventId:event.id}});
